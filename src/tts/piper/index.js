    ...
    createPiperPhonemize = (() => {
      ...
      return (function(moduleArg = {}) {
          ...
          var loadPackage = function(metadata) {
            ...
            function fetchRemotePackage(packageName, packageSize, callback, errback) {
              if (typeof process === "object" && typeof process.versions === "object" && typeof process.versions.node === "string") {
                (init_empty(), __toCommonJS(empty_exports)).readFile(packageName, function(err2, contents) {
                  if (err2) {
                    errback(err2);
                  } else {
        ...
        if (ENVIRONMENT_IS_NODE) {
          var fs2 = (init_empty(), __toCommonJS(empty_exports));
        ...
        var FS_createDataFile = (parent, name2, fileData, canRead, canWrite, canOwn) => FS.createDataFile(parent, name2, fileData, canRead, canWrite, canOwn);
        ...
        } }, major: (dev) => dev >> 8, minor: (dev) => dev & 255, makedev: (ma, mi2) => ma << 8 | mi2, registerDevice(dev, ops) {
        ...
        function ___syscall_ioctl(fd2, op2, varargs) {
          SYSCALLS.varargs = varargs;
          try {
            var stream = SYSCALLS.getStreamFromFD(fd2);
            switch (op2) {
        ...
        var convertI32PairToI53Checked = (lo2, hi2) => hi2 + 2097152 >>> 0 < 4194305 - !!lo2 ? (lo2 >>> 0) + hi2 * 4294967296 : NaN;
        ...
        var _strftime = (s, maxsize, format, tm2) => {
        ...
        var _strftime_l = (s, maxsize, format, tm2, loc) => _strftime(s, maxsize, format, tm2);
...
// src/voices_static.json
var require_voices_static = __commonJS({
  "src/voices_static.json"(exports2, module2) {
    module2.exports = {
      "en_US-homer-medium": { // DEFAULT ?
        key: "en_US-homer-medium",
        name: "ryan",
        language: {
          code: "en_US",
          family: "en",
...
var TtsSession = class _TtsSession {
  static WASM_LOCATIONS = DEFAULT_WASM_PATHS;
  static _instance = null;
  ready = false;
  voiceId = "en_US-richie-medium";
  waitReady = false;
  #createPiperPhonemize;
  #modelConfig;
  #ort;
  ...
  constructor({
    ...
    if (_TtsSession._instance) {
      ...
      _TtsSession._instance.#progressCallback = progress ?? _TtsSession._instance.#progressCallback;
  ...
  async init() {
    const { createPiperPhonemize: createPiperPhonemize2 } = await Promise.resolve().then(() => (init_piper(), piper_exports));
    this.#createPiperPhonemize = createPiperPhonemize2;
    this.#ort = await Promise.resolve().then(() => (init_ort_min(), ort_min_exports));
  ...
  async predict(text) {
    ...
    const phonemeIds = await new Promise(async (resolve) => {
      const module2 = await this.#createPiperPhonemize({
        print: (data) => {
...
async function stored() {
  ...
  const result = [];
  for await (const name2 of dir.keys()) {
    const key = name2.split(".")[0];
    if (name2.endsWith(".onnx") && key in PATH_MAP) {
      result.push(key);
    }
  }
  return result;
...
async function voices() {
  try {
    ...
    const LOCAL_VOICES_JSON = await Promise.resolve().then(() => __toESM(require_voices_static(), 1));
